(function () {
    ...
    addEvent(password, 'input', function (e) { 
        var target = e.target || e.srcElement;
        submit.disabled = submitted || !target.value || target.value.length < 10;  
      });

      addEvent(password2, 'input', function (e) { 
        var target = e.target || e.srcElement;
        submit.disabled = submitted || !target.value || password.value != target.value;  
      });

      addEvent(accept, 'input', function (e) { 
        var target = e.target || e.srcElement;
        submit.disabled = submitted || !target.checked;  
      });

    addEvent(country, 'focus', function () {
    var options = '<option selected>Please choose a country</option>';
    var key;
    for (key in countries) {                  
      options += `<option value="${countries[key].code}">${countries[key].name}</option>`;
    } 
    country.innerHTML = options; 
});

addEvent(country, 'input', function (e) { 
    var target = e.target || e.srcElement;
    submit.disabled = submitted || !target.value;
  });
    ...
    addEvent(form, 'submit', function (e) {             // When user submits form
        ...
        var code = elements.country.value;        
        var msg = 'Welcome ' + username  + '! The country code is ' + code;            //  welcome message
